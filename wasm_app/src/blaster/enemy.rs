...
pub struct BlasterEnemy {
    ...
    pub max_hp: i32,
    pub shoot_timer: f32,
    pub body_angle:   f32,   // 車体の向き（移動方向）
    pub turret_angle: f32,   // 砲塔の向き（プレイヤー方向・独立回転）
...
impl BlasterEnemy {
    pub fn new_basic(x: f32, z: f32) -> Self {
        Self { active: true, kind: EnemyKind::Basic, x, y: 0.5, z,
               hp: 2, max_hp: 2, shoot_timer: 0.0,
               body_angle: 0.0, turret_angle: 0.0, vx: 0.0, vz: 0.0 }
    }
    pub fn new_shooter(x: f32, z: f32) -> Self {
        Self { active: true, kind: EnemyKind::Shooter, x, y: 0.5, z,
               hp: 4, max_hp: 4, shoot_timer: 1.5,
               body_angle: 0.0, turret_angle: 0.0, vx: 0.0, vz: 0.0 }
    }
}
