...
pub fn boot_cartridge_msx() {
    MSX.with(|s| {
        if let Some(m) = s.borrow_mut().as_mut() {
            ...
            // Read INIT address from offset 2-3
            let init_low = m.bus.cart[2] as u16;
            let init_high = m.bus.cart[3] as u16;
...
thread_local! {
    static PENPEN: std::cell::RefCell<Option<penpen::PenPenGame>> = std::cell::RefCell::new(None);
}
...
#[wasm_bindgen]
pub fn resize_penpen(w: u32, h: u32) {
    PENPEN.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            g.gpu.resize(w, h);
        }
...
#[wasm_bindgen]
pub fn score_penpen() -> u32 {
    PENPEN.with(|s| s.borrow().as_ref().map(|g| g.score).unwrap_or(0))
}

#[wasm_bindgen]
pub fn speed_penpen() -> f32 {
    PENPEN.with(|s| s.borrow().as_ref().map(|g| g.player_speed).unwrap_or(0.0))
}

#[wasm_bindgen]
pub fn hp_penpen() -> i32 {
    PENPEN.with(|s| s.borrow().as_ref().map(|g| g.player_hp).unwrap_or(0))
}

#[wasm_bindgen]
pub fn max_hp_penpen() -> i32 {
    PENPEN.with(|s| s.borrow().as_ref().map(|g| g.player_max_hp).unwrap_or(5))
}

#[wasm_bindgen]
pub fn level_penpen() -> u32 {
    PENPEN.with(|s| s.borrow().as_ref().map(|g| g.level).unwrap_or(1))
}

#[wasm_bindgen]
pub fn fish_count_penpen() -> u32 {
    PENPEN.with(|s| s.borrow().as_ref().map(|g| g.fish_collected).unwrap_or(0))
}

#[wasm_bindgen]
pub fn audio_event_penpen() -> u8 {
    PENPEN.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            let ev = g.audio_event;
            g.audio_event = 0; // 消費する
            ev
        } else {
            0
        }
...
#[wasm_bindgen]
pub fn sound_def_penpen(event: u8) -> String {
    penpen::sound_def(event)
}

#[wasm_bindgen]
pub fn progress_penpen() -> f32 {
    PENPEN.with(|s| {
        s.borrow().as_ref()
            .map(|g| (g.player_z / g.track_length).clamp(0.0, 1.0))
            .unwrap_or(0.0)
    })
}

#[wasm_bindgen]
pub fn switch_camera_penpen() {
    PENPEN.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            g.camera_mode = (g.camera_mode + 1) % 3;
        }
    });
}

#[wasm_bindgen]
pub fn camera_name_penpen() -> String {
    PENPEN.with(|s| {
        s.borrow().as_ref()
            .map(|g| match g.camera_mode {
                1 => "TOP".to_string(),
                2 => "FPS".to_string(),
                _ => "TPS".to_string(),
            })
...
thread_local! {
    static PENPEN2: std::cell::RefCell<Option<penpen2::PenPenGame>> = std::cell::RefCell::new(None);
}

#[wasm_bindgen]
pub async fn init_penpen2(canvas_id: &str) {
    console_error_panic_hook::set_once();
    let game = penpen2::PenPenGame::new(canvas_id).await.unwrap();
    PENPEN2.with(|s| *s.borrow_mut() = Some(game));
}

#[wasm_bindgen]
pub fn tick_penpen2(ts: f64) {
    PENPEN2.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            g.tick(ts);
            // レンダリング
            let aspect = g.gpu.width as f32 / g.gpu.height as f32;
            let (verts, idxs) = penpen2::build_penpen_scene(g);
            let uni = penpen2::build_penpen_uni(g, aspect);
            g.gpu.render(&verts, &idxs, &uni);
        }
    });
}
...
#[wasm_bindgen]
pub fn resize_penpen2(w: u32, h: u32) {
    PENPEN2.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            g.gpu.resize(w, h);
        }
...
#[wasm_bindgen]
pub fn set_accel_input_penpen2(on: bool) {
    PENPEN2.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            g.set_accel_input(on);
        }
    });
}

#[wasm_bindgen]
pub fn set_brake_input_penpen2(on: bool) {
    PENPEN2.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            g.set_brake_input(on);
        }
    });
}

#[wasm_bindgen]
pub fn pull_dist_penpen2() -> f32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.pull_dist).unwrap_or(0.0))
}

#[wasm_bindgen]
pub fn scene_penpen2() -> u8 {
    PENPEN2.with(|s| {
        s.borrow().as_ref()
            .map(|g| g.scene as u8)
            .unwrap_or(0)
...
#[wasm_bindgen]
pub fn score_penpen2() -> u32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.score).unwrap_or(0))
}

#[wasm_bindgen]
pub fn speed_penpen2() -> f32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.player_speed).unwrap_or(0.0))
}

#[wasm_bindgen]
pub fn hp_penpen2() -> i32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.player_hp).unwrap_or(0))
}

#[wasm_bindgen]
pub fn max_hp_penpen2() -> i32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.player_max_hp).unwrap_or(5))
}

#[wasm_bindgen]
pub fn level_penpen2() -> u32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.level).unwrap_or(1))
}

#[wasm_bindgen]
pub fn fish_count_penpen2() -> u32 {
    PENPEN2.with(|s| s.borrow().as_ref().map(|g| g.fish_collected).unwrap_or(0))
}

#[wasm_bindgen]
pub fn audio_event_penpen2() -> u8 {
    PENPEN2.with(|s| {
        if let Some(g) = s.borrow_mut().as_mut() {
            let ev = g.audio_event;
            g.audio_event = 0; // 消費する
...
#[wasm_bindgen]
pub fn sound_def_penpen2(event: u8) -> String {
    penpen2::sound_def(event)
}

#[wasm_bindgen]
pub fn progress_penpen2() -> f32 {
    PENPEN2.with(|s| {
        s.borrow().as_ref()
            .map(|g| (g.player_z / g.track_length).clamp(0.0, 1.0))
            .unwrap_or(0.0)
...
#[wasm_bindgen]
pub fn camera_name_penpen2() -> String {
    PENPEN2.with(|s| {
        s.borrow().as_ref()
            .map(|g| match g.camera_mode {
                1 => "TOP".to_string(),
